...
public class EspecialidadeController {
    ...
    @GetMapping("/{codigo}")
    public ResponseEntity<EspecialidadeModel> findByCodigo(@PathVariable String codigo) {
        return especialidadeService.findByCodigo(codigo)
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }
    
    @PostMapping
    public ResponseEntity<EspecialidadeModel> create(@RequestBody EspecialidadeModel especialidade) {
        return ResponseEntity.status(HttpStatus.CREATED).body(especialidadeService.save(especialidade));
    }
    
    @PutMapping("/{codigo}")
    public ResponseEntity<EspecialidadeModel> update(@PathVariable String codigo, @RequestBody EspecialidadeModel especialidade) {
